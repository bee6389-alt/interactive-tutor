...
export function setLearnerMode(state, mode) {
...
export function evaluateAnswer(expected, accepted, rawAnswer) {
...
export function getQuestionBankSummary(course) {
...
export function getCurrentVideoResources(course, state) {
  const chapter = course.chapters[state.chapterIndex];
  const step = getCurrentStep(course, state);
...
export function getCurrentLessonGate(course, state) {
  const chapter = course.chapters[state.chapterIndex];
  const step = getCurrentStep(course, state);
  if (!chapter || !step) {
...
export function markCurrentLessonVideoComplete(course, state) {
...
export function startPracticeQuestion(course, state, mode) {
  const banks = createQuestionBanks(course);
...
export function handlePracticeAnswer(course, state, answer) {
...
export function handleStudentAnswer(course, state, answer) {
  const step = getCurrentStep(course, state);
  if (!step) {
    return {
      state,
...
export function summarizeProgress(course, state) {
