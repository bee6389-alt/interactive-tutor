const tutorEngine = window.TutorEngine;

const STORAGE_KEY = "basic-electricity-tutor-state-v1";
...
elements.answerForm.addEventListener("submit", (event) => {
  ...
  const result = state.activePracticeQuestion
    ...
    : tutorEngine.handleStudentAnswer(tutorEngine.COURSE, state, answer);
  state = result.state;
  saveState();
...
elements.resetButton.addEventListener("click", () => {
  ...
  state = tutorEngine.createInitialState();
  saveState();
...
elements.exportButton.addEventListener("click", () => {
  const payload = {
    ...
    course: tutorEngine.COURSE.title,
    summary: tutorEngine.summarizeProgress(tutorEngine.COURSE, state),
    state,
  };
  ...
  link.href = url;
  link.download = "basic-electricity-student-progress.json";
  link.click();
  URL.revokeObjectURL(url);
});

elements.practiceButton.addEventListener("click", () => {
  startPracticeMode("practice");
});

elements.examButton.addEventListener("click", () => {
  startPracticeMode("exam");
});

elements.remediationButton.addEventListener("click", () => {
  startPracticeMode("remediation");
});

function startPracticeMode(mode) {
  const result = tutorEngine.startPracticeQuestion(tutorEngine.COURSE, state, mode);
  state = result.state;
  saveState();
...
function getHandwritingContext() {
  const currentChapter = tutorEngine.COURSE.chapters[state.chapterIndex];
  const currentStep = currentChapter?.steps?.[state.stepIndex];
  const activeQuestion = state.activePracticeQuestion;
  return {
    subject: "basic-electricity",
    courseTitle: tutorEngine.COURSE.title,
    chapterTitle: currentChapter?.title || "",
    question: activeQuestion?.prompt || tutorEngine.getCurrentPrompt(tutorEngine.COURSE, state),
...
function loadState() {
  try {
    ...
    if (!stored) return tutorEngine.createInitialState();
    return { ...tutorEngine.createInitialState(), ...JSON.parse(stored) };
  } catch {
    return tutorEngine.createInitialState();
  }
}
...
function renderAll() {
  const summary = tutorEngine.summarizeProgress(tutorEngine.COURSE, state);
  const bankSummary = tutorEngine.getQuestionBankSummary(tutorEngine.COURSE);
  elements.overallPercent.textContent = `${summary.percent}%`;
  elements.chapterCounter.textContent = `第 ${Math.min(state.chapterIndex + 1, tutorEngine.COURSE.chapters.length)} / ${tutorEngine.COURSE.chapters.length} 章`;
...
function renderChapters() {
  ...
  tutorEngine.COURSE.chapters.forEach((chapter, index) => {
...
function selectChapter(index) {
  if (index < 0 || index >= tutorEngine.COURSE.chapters.length) return;
...
function renderVideoResources() {
  ...
  const resources = tutorEngine.getCurrentVideoResources(tutorEngine.COURSE, state);
...
function renderConversation() {
  ...
  if (currentTranscript.length === 0) {
    ...
    addTutorMessage(tutorEngine.getCurrentPrompt(tutorEngine.COURSE, state));
    return;
  }
...
function addResultMessages(messages) {
  const gate = tutorEngine.getCurrentLessonGate(tutorEngine.COURSE, state);
  const currentPrompt = tutorEngine.getCurrentPrompt(tutorEngine.COURSE, state);
  const locked = isLessonLocked(gate);
